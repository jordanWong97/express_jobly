...
describe("GET /users", function () {
  ...
  test("unauth for non-admin or wrong user", async function () {
    const resp = await request(app)
      .get("/users")
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("GET /users/:username", function () {
  ...
  test("works for appropriate user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works: set new password as appropriate user", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
...
describe("DELETE /users/:username", function () {
  ...
  test("auth for non-admin but appropriate user", async function () {
    const resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ deleted: "u1" });
